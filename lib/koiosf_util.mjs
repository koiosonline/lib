...
export async function FetchIPFSTxt (cid) {
   ...
   // console.log(`Cid: ${cid} : ${resultarray.length}`)    
    
    for (var i=0;i<resultarray.length;i++){     
        str +=resultarray[i].toString()                    
    }
    
    //console.log(`Cid: ${cid} : ${str.length}`)
    return str;
}
...
export async function GetJsonIPFS(cid) { // withouth gateway, currently error
    if (!cid) return undefined;
    cid=stripipfsprefix(cid)
...
async function loadipfs() {
    await loadScriptAsync("https://unpkg.com/ipfs@0.54.2/dist/index.min.js");
    ...
    console.log(`Connect ${con2}`)
    await ipfs.swarm.connect(con2).catch(console.log); 
