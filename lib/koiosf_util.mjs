...
export async function FetchIPFSUint8Array (cid) {
    cid=stripipfsprefix(cid)
    var resultarray=[]
    var ipfs = await promiseloadipfs;
...
export async function FetchIPFSTxt (cid) {
    cid=stripipfsprefix(cid)
    var ipfs = await promiseloadipfs;
    var str=""
    
    console.log("In FetchIPFSTxt");


    for await (const result of ipfs.cat(cid,{length:100000})) {     // not too large other wise fromCharCode.apply give stack error
        console.log(result.length);
        //if (result.length > 100000) return "far too large, probably not text";
        try { 
            str += String.fromCharCode.apply(null, result); // convert uint8array to string                        
        } catch (error) { console.log(`Error in fromCharCode :${cid} ${error}`); return "";}
    }         
    //console.log(`Cid: ${cid} : ${str}`)
    return str;
}
...
export async function GetJsonIPFS(cid) {
    if (!cid) return undefined;
    cid=stripipfsprefix(cid)
    var indexjson=await FetchIPFS(cid)     //FetchIPFSTxt
    if (!indexjson)
        return undefined;    
    //var json = JSON.parse(indexjson)
    var json = await indexjson.json();
    return json;
}    


export async function GetJsonIPFS2(cid) { // withouth gateway, currently error
    if (!cid) return undefined;
    cid=stripipfsprefix(cid)
    var indexjson=await FetchIPFSTxt(cid)     
    if (!indexjson)
        return undefined;    
    var json = JSON.parse(indexjson)   
    return json;
...
export async function GetImageIPFS(cid) {
    ...
    */
    //console.log(`GetImageIPFS ${cid}`)
    var ui8arr=await FetchIPFSUint8Array(cid)
    var blob = new Blob( ui8arr ,{ type: (ui8arr[0][0] == 60)?"image/svg+xml":"image/jpeg" }); // 60 == "<"
    //console.log(`First char: ${ui8arr[0][0]}`)
    // image/svg+xml
    //console.log(blob);    
