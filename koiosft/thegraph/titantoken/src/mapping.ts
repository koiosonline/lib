...
export function handleTransfer(event: Transfer): void {
  ...
  changeUser(erc20Symbol, contentURI, event.params.from.toHex(), -event.params.value);
  changeUser(erc20Symbol, contentURI, event.params.to.toHex(), event.params.value);
}

function changeUser(erc20Symbol: string, contentURI: string, address: string, delta: BigInt): void { // note delta can be neg.
